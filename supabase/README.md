...
4. Ensure Storage bucket **`avatars`** exists and is **public** (same as client avatars). Worker files are stored under `workers/{user_id}/...`.
5. Add a storage policy allowing authenticated users to upload to `workers/{user_id}/**` if uploads are denied by default.
